...
uint8_t DHT_Check_Response(void)
{
	...
	{
		timeout++;
		if (timeout % 1000 == 0) // in ra moi 1000 lan lap
		{
			char msg[64];
			sprintf(msg, "PA0 van HIGH, dem = %lu\r\n", (unsigned long)timeout);
			HAL_UART_Transmit(&huart1, (uint8_t *)msg, strlen(msg), 100);
		}

		if (timeout > 50000) // neu qua lau thi thoat
		{
			HAL_UART_Transmit(&huart1, (uint8_t *)"Timeout DHT response\r\n", 23, 100);
			return 0;
		}
	}
